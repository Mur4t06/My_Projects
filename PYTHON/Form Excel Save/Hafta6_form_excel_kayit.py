...
def veriekle():
    ...
    if onay =="onaylandi":
        ...
        dosya_yolu1 = "/Users/m1mac/Desktop/data.xlsx"   
        #dosya_yolu2 = "~/Desktop/data.xlsx"   
        
        if not os.path.exists(dosya_yolu1):
            workbook = openpyxl.Workbook()
            sayfa= workbook.active
            basliklar = ["Adı", "Soyadı", "Bölüm", "Yaş", "Uyruk", "Ders Kaydı", "Sınıf", "Dönem"]
            sayfa.append(basliklar)
            workbook.save(dosya_yolu1)
        
        workbook = openpyxl.load_workbook(dosya_yolu1)
        sayfa=workbook.active
        sayfa.append([adi, soyadi, bolum, yas,uyruk,dk,sinif,donem])
        workbook.save(dosya_yolu1)
        
            
        tk.messagebox.showwarning(title="onay işi tamam", 
                            message="Bilgiler Dosyaya Kaydedildi")
    else:
